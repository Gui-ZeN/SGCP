...
/** Quanto já foi contado na tarefa, somando todas as pessoas e dias. */
export function totalContado(id: string, diarios: Pick<Diario, 'contagens'>[]): number {
  return diarios.reduce((t, d) => t + Math.max(0, Math.floor(Number(d.contagens?.[id])) || 0), 0);
}
